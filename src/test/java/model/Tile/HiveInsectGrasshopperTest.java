...
class HiveInsectGrasshopperTest {
    ...
    @Test
    void testInsectMoveReturnsTheExpectedMoveWhenMakingAValidMove() throws Hive.IllegalMove {
        ...
        ArrayList<HiveLocation> validPath = hiveInsectGrasshopper.move(-1,1,1,-1);
    ...
    // a Een sprinkhaan verplaatst zich door in een rechte lijn een sprong te maken naar een veld meteen achter een andere steen in de richting van de sprong.
    @Test
    void testInsectMoveThrowsExceptionWhenAStraightJumpFromTheHopperIsNotPossibleToCell() throws Hive.IllegalMove {
        HiveGame hiveGame = new HiveGame();
        HiveBoard hiveBoard = hiveGame.getBoard();
        hiveGame.play(Hive.Tile.QUEEN_BEE, 0,0); // wit
        hiveGame.play(Hive.Tile.QUEEN_BEE, 1, 0); // zwart
        ...
        assertThrows(IllegalMoveGrasshopper.class, ()-> {
            ArrayList<HiveLocation> validPath1 = hiveInsectGrasshopper.move(-2,2,0,1);
        });
        assertThrows(IllegalMoveGrasshopper.class, ()-> {
            ArrayList<HiveLocation> validPath2 = hiveInsectGrasshopper.move(-2,2,-1,0);
        });
        ArrayList<HiveLocation> validPath3 = hiveInsectGrasshopper.move(-2,2,1,-1);
        assertTrue(validPath3.get(0).equals(new HiveLocation(1, -1)));
    }

     // c Een sprinkhaan moet over minimaal één steen springen.
     @Test
     void testInsectMoveThrowsExceptionWhenTheJumpIsNotOverAtLeastOneTile() throws Hive.IllegalMove {
         HiveGame hiveGame = new HiveGame();
         HiveBoard hiveBoard = hiveGame.getBoard();
         hiveGame.play(Hive.Tile.QUEEN_BEE, 0,0); // wit
         hiveGame.play(Hive.Tile.QUEEN_BEE, 1, 0); // zwart
         ...
         HiveInsectGrasshopper hiveInsectGrasshopper = new HiveInsectGrasshopper(hiveGame, hiveBoard);
         assertThrows(IllegalMoveGrasshopper.class, ()->{
             ArrayList<HiveLocation> validPath1 = hiveInsectGrasshopper.move(0,1,1,1); // een cel verder
         });
         assertThrows(IllegalMoveGrasshopper.class, ()->{
             ArrayList<HiveLocation> validPath2 = hiveInsectGrasshopper.move(0,1,2,1); // 2 cel verder maar geen tile ertussen
         });
     }

     // e Een sprinkhaan mag niet over lege velden springen. Dit betekent dat alle velden tussen de start- en eindpositie bezet moeten zijn.
     @Test
     void testInsectMoveThrowsExceptionWhenTheJumpContainsEmptyFieldInBetween() throws Hive.IllegalMove {
         HiveGame hiveGame = new HiveGame();
         HiveBoard hiveBoard = hiveGame.getBoard();
         hiveGame.play(Hive.Tile.QUEEN_BEE, 0,0); // wit
         hiveGame.play(Hive.Tile.QUEEN_BEE, 1, 0); // zwart
         ...
         HiveInsectGrasshopper hiveInsectGrasshopper = new HiveInsectGrasshopper(hiveGame, hiveBoard);
         assertThrows(IllegalMoveGrasshopper.class, ()->{
             ArrayList<HiveLocation> validPath2 = hiveInsectGrasshopper.move(0,1,2,1); // 2 cel verder maar er is een lege cell ertussen
         });
     }
    ...
    // d Een sprinkhaan mag niet naar een bezet veld springen.
    @Test
    void testInsectMoveThrowsExceptionWhenToFieldIsNotEmpty() throws Hive.IllegalMove {
        HiveGame hiveGame = new HiveGame();
        HiveBoard hiveBoard = hiveGame.getBoard();
        hiveGame.play(Hive.Tile.QUEEN_BEE, 0,0); // wit
        hiveGame.play(Hive.Tile.QUEEN_BEE, 1, 0); // zwart
        ...
        HiveInsectGrasshopper hiveInsectGrasshopper = new HiveInsectGrasshopper(hiveGame, hiveBoard);
        assertThrows(IllegalMoveGrasshopper.class, ()-> {
            ArrayList<HiveLocation> validPath = hiveInsectGrasshopper.move(-2,2,0,0);
        });
    }
}